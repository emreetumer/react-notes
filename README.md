# 📚 React Notları ve Proje Örnekleri

Modern React geliştirme için kapsamlı Türkçe kaynak ve örnek projeler.

## 🎯 İçerik

Bu repo, React öğreniminde ihtiyaç duyacağınız tüm temel konuları ve pratik örnekleri içerir.

### 📖 Dersler

1. **[React Temelleri](React/dersler/ders-1-react-temelleri.md)** - Component yapısı, JSX temelleri
2. **[JSX ve Props](React/dersler/ders-2-jsx-ve-props.md)** - Props kullanımı, data aktarımı
3. **[State ve Events](React/dersler/ders-3-state-ve-events.md)** - useState hook'u, event handling
4. **[useEffect](React/dersler/ders-4-useEffect.md)** - Side effects, lifecycle
5. **[Forms ve Validation](React/dersler/ders-5-forms-validation.md)** - Form yönetimi, doğrulama
6. **[Custom Hooks](React/dersler/ders-6-custom-hooks.md)** - Özel hook'lar oluşturma
7. **[Context API](React/dersler/ders-7-context-api.md)** - Global state yönetimi
8. **[React Router](React/dersler/ders-8-react-router.md)** - Sayfa yönlendirme
9. **[Best Practices](React/dersler/ders-9-best-practices.md)** - En iyi uygulamalar

### 💻 Örnek Projeler

#### React Örnekleri
- **40+ Örnek Kod** - Her ders için detaylı örnekler
- Basit componentlerden karmaşık uygulamalara
- Tüm örnekleri [ORNEKLER-LISTE.md](React/ORNEKLER-LISTE.md) dosyasında bulabilirsiniz

#### Final Proje
Kapsamlı bir React uygulaması:
- API entegrasyonu
- Context API kullanımı
- Custom hooks
- React Router implementasyonu

[Final Proje Detayları](React/FINAL-PROJE/README.md)

#### Next.js Projesi
Modern Next.js uygulaması:
- TypeScript
- Tailwind CSS
- Server Components
- App Router

[Next.js Proje Detayları](React/NEXTJS-PROJE/README.md)

### 🚀 Örnek Dersleri

Temel React konularını uygulamalı olarak öğrenmek için:
- **1.Giris** - İlk React uygulaması
- **3.props** - Props ile çalışma
- **4-useState** - State yönetimi

## 🛠️ Kurulum

Projeleri çalıştırmak için:

```bash
# Proje klasörüne gidin
cd React/FINAL-PROJE  # veya istediğiniz proje

# Bağımlılıkları yükleyin
npm install

# Geliştirme sunucusunu başlatın
npm run dev
```

## 📋 Gereksinimler

- Node.js (v16 veya üzeri)
- npm veya yarn
- Modern bir web tarayıcısı

## 🎓 Nasıl Kullanılır?

1. İlk olarak [dersler](React/dersler/) klasöründeki notları sırasıyla okuyun
2. Her ders sonrasında ilgili örnek kodları inceleyin
3. Örnek projeleri çalıştırarak pratik yapın
4. Final proje ile öğrendiklerinizi pekiştirin

## 📝 Özellikler

- ✅ Türkçe kaynak
- ✅ Güncel React (Hooks API)
- ✅ Pratik örnekler
- ✅ Gerçek proje senaryoları
- ✅ Best practices
- ✅ TypeScript örnekleri

## 🤝 Katkıda Bulunma

Bu notlar sürekli geliştirilmektedir. Katkılarınızı bekliyorum!

## 📧 İletişim

Sorularınız için issue açabilirsiniz.

## 📄 Lisans

Bu proje eğitim amaçlıdır ve özgürce kullanılabilir.

---

**Not:** Bu repo, React öğrenmek isteyen herkes için hazırlanmıştır. Temel JavaScript bilgisi önerilir.
